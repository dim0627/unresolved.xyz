...
honn.meは本の推薦文を共有するサービスで、ユーザーの行動をDBのイベントログとして記録しています。Google Analyticsでページビューは見れますが、「ホームを見た人のうち何人がタイトル入力まで進んだか」「本を選んだ人のうち何人が作成ボタンを押したか」といったファネル分析はGA単体では難しい。

DBには `page_view_home → title_input_focus → book_select → create_button_click → share_button_click` というイベントが記録されているので、これを使ってファネル分析を自動化したいと考えました。
...
- **Claude Code駆動**: スクリプトの実装からSQLの組み立てまで、Claude Codeに主導してもらう。自分は方針を決めて指示を出す側に徹する。さらに、収集済みデータの分析もClaude Codeに任せる。GitHub Actionsが毎日記録した定点データをClaude Codeに読ませて「この数字どう思う？」と対話する使い方
...
- **TypeScript + SQL二段構え**: データ取得スクリプトはTypeScript（tsx）で書き、複雑なクエリはSQLで管理する
...
ファネルを組んでデータを取得してみたら、`share_button_click` のカウントが0。調べてみると、そもそもフロントエンドでこのイベントを送信する実装がされていませんでした。分析基盤を作ったことで、計測漏れを発見できたわけです。これは嬉しい副産物でした。
...
前述の `share_button_click` の計測漏れもそうですが、ファネルを定義する過程で「このステップの間に何が起きているのか」を深く考えることになりました。分析基盤を作ること自体が、プロダクトの解像度を上げる行為だったと感じています。
...
データを見ていて `book_remove` というイベントが気になりました。本を一度選択した後に削除するユーザーがいる。これは「間違えて選んだ」のか「比較検討している」のか。こういった解釈の余地がある指標をClaude Codeに投げて議論できるのは、AI駆動の分析基盤ならではの体験でした。
...
- 「先週と比べて転換率がどう変わったか」「どのステップにボトルネックがあるか」を分析
- 数字の解釈や改善仮説の提示
- 対話的に深掘りできる（「book_selectの離脱が多いのはなぜ？」など）
...
個人開発の規模感では、専用のBIツールを導入するほどでもないけれど、GAだけでは足りない。そのちょうど良い中間地点として、この構成はかなりフィットしていると感じています。honn.meで仕組みが回り始めたので、次は他のプロダクトにも同じ基盤を横展開していく予定です。

分析基盤を作る過程で計測漏れを発見したり、ファネルの転換率からUXの課題が見えたり、「作ること自体がプロダクト改善」という体験ができたのは、やってみて良かったポイントです。
